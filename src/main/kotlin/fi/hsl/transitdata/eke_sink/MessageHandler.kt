...
class MessageHandler(private val csvService: CsvService) : IMessageHandler {
    ...
    override fun handleMessage(received: Message<Any>) {
        try {
            ...
            // Time when the MQTT message was received (nullable because older versions of
            // mqtt-pulsar-gateway don't support this)
            val mqttTimestamp =
                received.properties[TransitdataProperties.KEY_SOURCE_MESSAGE_TIMESTAMP_MS]
                    ?.toLong()
                    ?.let { epochMilli -> Instant.ofEpochMilli(epochMilli) }
            ...
            if (++handledMessages == LOG_THRESHOLD) {
                log.info(
                    "Handled $LOG_THRESHOLD messages. Free memory: ${Runtime.getRuntime().freeMemory() / 1024 / 1024}MB"
                )
                handledMessages = 0
            }
        } catch (e: Exception) {
