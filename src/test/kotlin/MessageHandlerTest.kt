...
class MessageHandlerTest {
    ...
    fun before() {
        ...
        mockMessage =
            mock<TypedMessageBuilder<ByteArray>>(stubOnly = true) {
    ...
    fun handleMessageTest() {
        ...
        val handler =
            MessageHandler(
                CsvService(
                    directory,
                    LocalSink(uploadDirectory),
                    mockConsumer::acknowledgeAsync,
                    uploadAfterNotModified = Duration.ofSeconds(1),
                    tryUploadInterval = Duration.ofSeconds(2)
                )
            )
        handler.handleMessage(
            mock<Message<Any>> {
                on { data } doAnswer
                    {
                        Mqtt.RawMessage.newBuilder()
                            .setPayload(ByteString.copyFrom(getMessageContent()))
                            .setTopic("eke/v1/sm5/15/A/stadlerUDP")
                            .setSchemaVersion(1)
                            .build()
                            .toByteArray()
                    }
                on { properties } doReturn
                    (Collections.singletonMap(
                        fi.hsl.common.transitdata.TransitdataProperties
                            .KEY_SOURCE_MESSAGE_TIMESTAMP_MS,
    ...
    private fun getMessageContent(): ByteArray =
        this::class.java.getResourceAsStream("stadlerUDP.dat").use { it.readBytes() }
}
